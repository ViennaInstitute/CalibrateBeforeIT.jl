...
result_figaro = CBit.combine_tables(eurostat_path; conn=conn, skip_if_missing=false)
...
sqlquery = """
COPY (
SELECT * FROM '$(CBit.pqfile("une_rt_a"))'
WHERE value IS NOT NULL
)
TO '$(CBit.pqfile("une_rt_a"))' (FORMAT parquet)
"""
CBit.execute(conn,sqlquery);
...
SELECT * FROM '$(CBit.pqfile("une_rt_a_h"))'
WHERE value IS NOT NULL
)
SELECT * FROM table1
UNION
...
TO '$(CBit.pqfile("une_rt_a"))' (FORMAT parquet)
"""
res_une_rt_a = CBit.execute_debug(conn,sqlquery);
...
sqlquery = """
COPY (
SELECT * FROM '$(CBit.pqfile("une_rt_q"))'
WHERE value IS NOT NULL
)
TO '$(CBit.pqfile("une_rt_q"))' (FORMAT parquet)
"""
CBit.execute(conn,sqlquery);
...
SELECT * FROM '$(CBit.pqfile("une_rt_q_h"))'
WHERE value IS NOT NULL
)
SELECT * FROM table1
UNION
...
TO '$(CBit.pqfile("une_rt_q"))' (FORMAT parquet)
"""
res_une_rt_q = CBit.execute_debug(conn,sqlquery);
...
# test = "SELECT * FROM '$(CBit.pqfile("bd_9ac_l_form_a64"))' LIMIT 10;"
