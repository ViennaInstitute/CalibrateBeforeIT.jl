...
@testset "NACE Rev.2 to NACE64 Conversion Tests" begin
    ...
    @testset "create_sbs_na_sca_a64 validation" begin
        conn = DuckDB.DBInterface.connect(DuckDB.DB)
        
        # Test with nonexistent directory
        @test_throws ArgumentError CalibrateBeforeIT.create_sbs_na_sca_a64("nonexistent_dir", conn)
        
        # Test that function exists and has correct signature
        @test hasmethod(CalibrateBeforeIT.create_sbs_na_sca_a64, Tuple{String, Any})
        
        # Test with different excluded industries (keyword argument)
        try
            CalibrateBeforeIT.create_sbs_na_sca_a64("nonexistent", conn; excluded_industries=String[])
        catch ArgumentError
            # This is expected - we're just testing the method signature works
            @test true
        end
