...
"""
    create_sbs_na_sca_a64(save_path::String, conn;
                          excluded_industries::Vector{String} = ["L68A", "T", "U"])
...
conn = DBInterface.connect(DuckDB.DB)
save_path = "data/010_eurostat_tables"
success, rows = create_sbs_na_sca_a64(save_path, conn)
```
"""
function create_sbs_na_sca_a64(save_path::String, conn;
                               excluded_industries::Vector{String} = ["L68A", "T", "U"])

    # Input validation
    nace64_file = joinpath(save_path, "nace64.parquet")
    input_file = joinpath(save_path, "sbs_na_sca_r2.parquet")
    output_file = joinpath(save_path, "sbs_na_sca_a64.parquet")
    ...
    try
        ...
        COPY (WITH sbs AS (
            WITH nace64 AS (SELECT * FROM '$nace64_file')
            SELECT nace, geo, indic_sb, time, sum(value) AS value
            FROM '$input_file'
            JOIN nace64 ON nace_r2::text ~ nace64.regex
            WHERE nace_r2 NOT IN (SELECT nace FROM nace64)
              AND nace NOT IN (SELECT nace_r2 FROM '$input_file')
            ...
            UNION
            SELECT nace, geo, indic_sb, time, value
            FROM '$input_file'
            JOIN nace64 ON nace_r2=nace64.nace
        ), foo AS (
            WITH sbs_geo AS (SELECT DISTINCT geo FROM '$input_file'),
                 ...
                 sbs_time AS (SELECT DISTINCT time FROM '$input_file'),
                 nace64 AS (SELECT * FROM '$nace64_file')
            SELECT nace64.nace, sbs_geo.geo, sbs_indic.indic_sb, sbs_time.time
            FROM nace64, sbs_geo, sbs_indic, sbs_time
        )
        SELECT foo.nace AS nace_r2, foo.geo, foo.indic_sb, foo.time, value
        FROM foo
        LEFT JOIN sbs ON foo.nace=sbs.nace AND foo.geo=sbs.geo AND foo.indic_sb=sbs.indic_sb
        ...
        @info "Successfully created sbs_na_sca_a64.parquet" rows_processed

        return (true, rows_processed)

    catch e
        @error "Failed to create sbs_na_sca_a64" error=e
        return (false, 0)
    end
end
